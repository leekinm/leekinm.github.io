...
(function (global, factory) {
  ...
  ***************************************************************************** */
  ...
  function observe(value, shallow, ssrMockReactivity) {
    ...
    // Tips:这里还可以通过shouldObserve控制数据是否需要响应式
    else if (
      shouldObserve &&
  ...
  function defineReactive(obj, key, val, customSetter, shallow, mock) {
    ...
    Object.defineProperty(obj, key, {
      ...
      set: function reactiveSetter(newVal) {
        ...
        // 这个customSetter在`initInjections`中有用到，目的是
        // 如果试图直接通过修改inject数据，则直接打印警告提示：不得直接修改inject数据，
        // 因为有可能受父组件/祖先组件的重新渲染导致修改的inject数据又被重置了
        if (customSetter) {
          customSetter();
        }
        if (setter) {
  ...
  function updateListeners(on, oldOn, add, remove, createOnceHandler, vm) {
    ...
    // 如果在旧事件map里，且不在新的事件map里，则移除旧事件监听器
    for (name in oldOn) {
      if (isUndef(on[name])) {
        event = normalizeEvent(name);
  ...
  function initEvents(vm) {
    // 创建一个不包含prototype的空对象
    vm._events = Object.create(null);
    // 可以通过类似 this.$emit('hook:message') 调用事件监听器
  ...
  function updateComponentListeners(vm, listeners, oldListeners) {
    // 当前组件实例，在绑定和卸载事件监听器时会用到
    target$1 = vm;
    updateListeners(
      listeners,
      oldListeners || {},
      // 因为以下3个方法都需要用到vm，但是这3个方法又是跟updateListeners平级的
      // 所以只能用一个变量将vm保存下来，待使用完毕，则置空
      add$1,
      remove$1,
      createOnceHandler$1,
      vm
  ...
  function eventsMixin(Vue) {
    ...
    Vue.prototype.$on = function (event, fn) {
      var vm = this;
      // $on也可以监听一个数组，比如：this.$on(['evt1', 'evt2'], cb)
      // evt1和evt2触发时，都会执行cb
      if (isArray(event)) {
      ...
      // 如果是非数组的事件名，则将事件名作为key，并初始化为一个数组，然后将
      // 同类型的事件添加到该数组中
      // 比如this.$on('clickTitle', fn1)、this.$on('clickTitle', fn2)
      // vm._events['clickTitle'] = [fn1, fn2]
      else {
  ...
  if (typeof Promise !== "undefined" && isNative(Promise)) {
    ...
    timerFunc = function () {
      counter_1 = (counter_1 + 1) % 2; // TODO: 为啥要搞成0？
  ...
  function initProvide(vm) {
    ...
    if (provideOption) {
      ...
      for (var i = 0; i < keys.length; i++) {
        var key = keys[i];
        // 修改原有对象的属性，注意到provide传递的数据是不支持响应式的
        Object.defineProperty(
          source,
          key,
          /* 比如{value:'xxx', writable:true, enumerable:true, configurable:true} */
  ...
  function initInjections(vm) {
    ...
    if (result) {
      ...
      Object.keys(result).forEach(function (key) {
        /* istanbul ignore else */
        {
          // inject接收的数据都会添加到响应式系统中，使得provide提供的数据一旦发生变化，
          // inject对应的组件能够及时做出更新
  ...
  function resolveInject(inject, vm) {
    if (inject) {
      ...
      for (var i = 0; i < keys.length; i++) {
        ...
        // from对应的Key必须在provide中找得到，否则不起作用
        var provideKey = inject[key].from;
        if (provideKey in vm._provided) {
          result[key] = vm._provided[provideKey];
          // 如果没有提供from，存在default，则直接用default返回的值
  ...
  function FunctionalRenderContext(data, props, children, parent, Ctor) {
    ...
    this.children = children;
    this.parent = parent;
    this.listeners = data.on || emptyObject; // 保存着所有事件监听器
  ...
  function createPatchFunction(backend) {
    ...
    function invokeDestroyHook(vnode) {
      var i, j;
      // 这里的data就是虚拟DOM上面的属性值，
      // 有key、class、ref、事件监听器等
      // 具体有哪些值可以看src/types/vnode.ts
      var data = vnode.data;
      if (isDef(data)) {
        // data.hook如果有值，说明该节点是一个组件
        // hook有这些值：init、desctory、insert、prepatch和postpatch等
        // 这里的意思是，如果是组件就执行destroy
    ...
    function updateChildren(
      ...
      while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
        ...
        // 比较新旧头节点是否相等
          ...
          // 更新Vnode上绑定的属性，如class、style、listeners、directives等
          patchVnode(
            oldStartVnode,
            newStartVnode,
            insertedVnodeQueue,
        ...
        // 比较旧尾新头节点是否相等
          ...
          if (isUndef(oldKeyToIdx))
            // 保存旧节点的Key，用于跟新节点对比时使用
            oldKeyToIdx = createKeyToOldIdx(oldCh, oldStartIdx, oldEndIdx);
          idxInOld = isDef(newStartVnode.key)
            ? oldKeyToIdx[newStartVnode.key]
            : // 用新节点跟旧节点一一比较
              findIdxInOld(newStartVnode, oldCh, oldStartIdx, oldEndIdx);
          // 如果新旧Key不相等，说明新节点是新创建的，则创建新节点
          if (isUndef(idxInOld)) {
            // New element
            createElm(
              newStartVnode,
    ...
    function patchVnode(
      ...
      // 在对比Children时，如果发现他俩都指向同一个对象，那就是位置不需要变动，直接返回
      if (oldVnode === vnode) {
        return;
      }
      if (isDef(vnode.elm) && isDef(ownerArray)) {
      ...
      var i;
      var data = vnode.data;
      // 如果有，调用prepatch钩子，也就是开始prepatch
      // prepatch主要做了：更新子组件
      ...
      // 如果不是真实的DOM节点，则可能还会进入updateChildren或者直接更新了
      // 如果新节点是真实DOM节点，并且可以执行打补丁的操作了，则立即更新
      if (isDef(data) && isPatchable(vnode)) {
        ...
        // 如果存在，执行update钩子，也就是diff得到的差异更新到DOM时触发
      ...
      if (isUndef(vnode.text)) {
        ...
        // 新子节点存在
          ...
          {
            // 校验是否存在相同的key，如果存在，抛出警告
            checkDuplicateKeys(ch);
          }
          // 旧节点是文本节点，删除旧节点文本内容，新增新节点
          if (isDef(oldVnode.text)) nodeOps.setTextContent(elm, "");
          addVnodes(elm, null, ch, 0, ch.length - 1, insertedVnodeQueue);
        }
        // 旧子节点存在，新子节点不存在，删除旧子节点
        else if (isDef(oldCh)) {
          removeVnodes(oldCh, 0, oldCh.length - 1);
        }
        // 新子节点不存在，旧节点是文本节点，删除旧节点的文本内容
        else if (isDef(oldVnode.text)) {
          nodeOps.setTextContent(elm, "");
        }
      }
      // 新旧节点都是文本节点且不相等，则更新为新节点的文本内容
      else if (oldVnode.text !== vnode.text) {
        nodeOps.setTextContent(elm, vnode.text);
      }
      // 如果有，执行postpatch钩子，也就是patch结束时的hook
      if (isDef(data)) {
    ...
    return function patch(oldVnode, vnode, hydrating, removeOnly) {
      ...
      // 如果旧节点不存在，而新节点存在，则
      // 基于vnode创建新的节点，并添加到insertedVnodeQueue中
      if (isUndef(oldVnode)) {
      ...
      // 新旧节点都存在
        ...
        // 是否是真实DOM，只有真实DOM才有nodeType
        // 如果不是真实DOM且是同类型的，直接VDOM diff
  ...
  // 更新真实DOM上的事件监听，如：<div @click="xxx">Greeting</div>
